...
async def fetch_realtime_tick(symbol: str) -> float | None:
    """Giá khớp lệnh gần nhất qua GraphQL công khai của DNSE (nguồn tick, tách biệt
    với chart OHLC). Đây là nguồn ưu tiên cho "giá hiện tại" — nến OHLC ngày hôm nay
    chỉ dùng làm dự phòng vì không đảm bảo cập nhật liên tục trong phiên. CHỈ chấp
    nhận tick của đúng hôm nay (không lùi ngày tìm tick cũ), để tránh hiển thị giá
    phiên trước như đang khớp lệnh realtime. Không có tick hôm nay -> trả None."""
    symbol = symbol.upper().strip()
    if symbol == "VNINDEX":
        return None
    ...
    try:
        ...
        response.raise_for_status()
        data = response.json()
        ticks = (data.get("data") or {}).get("GetKrxTicksBySymbols", {}).get("ticks", [])
        if ticks:
            match_price = ticks[0].get("matchPrice")
            if match_price:
                return round(float(match_price) * 1000)
    except (httpx.HTTPError, ValueError, TypeError, KeyError, AttributeError) as exc:
        logger.warning("fetch_realtime_tick(%s) lỗi: %s", symbol, exc)
    return None


async def current_price(symbol: str, *, ttl: int = 90) -> tuple[float, bool]:
    """Giá hiện tại dùng cho /quote và định giá danh mục: ưu tiên tick realtime,
    fallback về giá đóng cửa/nến đang chạy trong OHLC nếu không lấy được tick
    (ngoài giờ giao dịch, lỗi mạng tạm thời...). Trả về (giá, is_realtime)."""
    realtime_price = await fetch_realtime_tick(symbol)
    if realtime_price is not None:
        return realtime_price, True
    series = await fetch(symbol, days=5, ttl=ttl)
    return series.price, False
