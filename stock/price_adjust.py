...
port từ repo Gemini (`stock/price_adjust.py`), đã bỏ phụ thuộc `report_format`
(chưa có ở vietassist) và tự format số inline.

Vì sao cần module này: `stock/market.py` KHÔNG biết chuỗi giá đang dùng là giá
đã điều chỉnh hay giá thô, và failover DNSE ↔ vnstock/VCI có thể khác chính
sách điều chỉnh nên cùng một mã có thể đổi hệ quy chiếu giá giữa hai lần phân
tích mà không có dấu hiệu.

Nếu chuỗi chưa điều chỉnh, ngày giao dịch không hưởng quyền tạo ra một gap GIẢ
trong dữ liệu: SMA20/50, RSI14 và support/resistance đều bị méo trong khi báo
cáo vẫn trình bày như thể số liệu sạch.

Hệ thống không có nguồn dữ liệu corporate action, nên cách xác định duy nhất
đáng tin là so gap với BIÊN ĐỘ giá tối đa một phiên của thị trường VN: HOSE
±7%, HNX ±10%, UPCOM ±15%, phiên chào sàn/giao dịch trở lại ±20%. Gap vượt xa
các mức đó không thể là biến động giao dịch bình thường.

Module này KHÔNG sửa bất kỳ con số nào của `stock/policy.py`: nó chỉ phát
hiện gap và sinh cảnh báo đưa vào payload cho LLM diễn giải.
"""
...
# được chỉ bằng giá.
_COMMON_RATIOS: tuple[tuple[str, float], ...] = (
...
def infer_is_adjusted(closes: list[float], dates: list[str] | None = None) -> bool | None:
...
def _describe(gap: PriceGap) -> str:
    ...
    text = (
        f"{when} {direction} {abs(gap.move_pct):.2f}% so với phiên trước "
        f"({_fmt_price(gap.prev_close)} -> {_fmt_price(gap.close)} VND)"
    )
    if gap.ratio_hint:
...
def build_note(symbol: str, source: str, gaps: list[PriceGap]) -> str:
    """Câu cảnh báo đưa vào payload cho LLM; chuỗi rỗng khi không có gì đáng nói."""
    if not gaps:
        return ""
    ...
    if certain:
        return (
            ...
            f" điều chỉnh. SMA20/50, RSI14 và support/resistance đang bị méo — KHÔNG"
            f" được dùng làm cơ sở kết luận mạnh.{scope} PHẢI nêu hạn chế dữ liệu này"
            " trong phần rủi ro."
        )
    return (
        ...
        " không hưởng quyền chưa được điều chỉnh. Khi diễn giải SMA/RSI/support-resistance"
        f" phải nói rõ độ tin cậy giảm vì lý do này.{scope}"
    )


def audit_series(symbol: str, source: str, closes: list[float], dates: list[str] | None = None) -> PriceAudit:
