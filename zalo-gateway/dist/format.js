...
const ZALO_BOLD = "b";
const ZALO_ITALIC = "i";
function mergeStyle(styles, next) {
 ...
 */
export function markdownToZalo(text) {
    ...
    const append = (value, style) => {
        ...
        if (style)
            mergeStyle(styles, { start, len: value.length, st: style });
    };
 ...
 * every chunk.
 */
export function splitFormattedMessage(message, limit = DEFAULT_ZALO_MAX_LEN) {
    ...
    while (offset < message.text.length) {
        ...
        offset = actualEnd;
        while (offset < message.text.length && /\s/u.test(message.text[offset]))
            offset += 1;
    }
    return chunks;
}
