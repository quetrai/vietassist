"""Gửi text NGUYÊN VĂN trong khối `<pre>` của Telegram — port từ repo Gemini
(`tg_format_codeblock.py`).

Vì sao tách khỏi `services/tg_format.py`: `tg_format.reply_rich()` convert
markdown-lite sang HTML inline (`<b>`, `<i>`, `<code>`). Cách đó KHÔNG bao giờ
sinh ra `<pre>` — mà Telegram chỉ hiện nút "Copy" cho khối `<pre>`. Tệ hơn,
prompt tạo ảnh tiếng Anh chứa rất nhiều dấu `*` và `_` (vd `--ar 4:5`,
`snake_case`) nên bị `_ITALIC_STAR_RE`/`_ITALIC_RE` của `tg_format.py` nuốt
mất ký tự, khiến prompt chép ra KHÁC với bản AI trả về — dùng cho `/prompt`
(cả nhập text lẫn gửi ảnh) để tránh đúng lỗi này.
...
# Telegram khi text chứa nhiều ký tự đặc biệt — cắt theo ký tự RAW (trước khi
# escape) để không bao giờ cắt ngang giữa 1 entity đã escape (vd "&am" + "p;").
_ESCAPE_COST = {"&": 5, "<": 4, ">": 4}
_ESCAPE_MAP = {"&": "&amp;", "<": "&lt;", ">": "&gt;"}
...
async def reply_code_block(message, text: str, *, max_len: int = 4096) -> None:
    ...
    Copy nhưng vẫn giữ đúng nội dung, không bao giờ lỗi im lặng)."""
