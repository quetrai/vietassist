"""Chuyển markdown-lite (Groq/OpenRouter/Gemini hay trả về **bold**, *italic*/_italic_,
`code`, gạch đầu dòng) sang HTML mà Telegram hỗ trợ, để hiển thị đẹp thay vì hiện
...
async def _send_chunks(
    send_fn: Callable[[str, str | None], Awaitable[None]], text: str, max_len: int
) -> None:
    ...
    (Groq/OpenRouter/Gemini đôi khi sinh ký tự lệch cặp)."""
...
async def reply_rich(message, text: str, *, max_len: int = 4096) -> None:
    ...
    async def _send(content: str, parse_mode: str | None) -> None:
...
async def send_rich(bot, chat_id: int, text: str, *, max_len: int = 4096) -> None:
    ...
    services/reminders.py, web.py), nơi không có object `message` để gọi reply_rich()."""
