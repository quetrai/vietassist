...
from services import tg_format_codeblock as cb
...
def test_giu_nguyen_dau_sao_va_gach_duoi():
    ...
    Anh chứa * và _ vẫn phải còn nguyên (không bị tg_format.py hiểu nhầm thành
    markdown bold/italic và nuốt mất ký tự)."""
...
async def test_reply_code_block_gui_html_pre():
    msg = FakeMessage()
    await cb.reply_code_block(msg, "nội dung prompt")
...
async def test_reply_code_block_roi_ve_plain_text_khi_html_loi():
...
async def test_reply_code_block_chia_nhieu_khoi_khi_qua_dai():
    msg = FakeMessage()
    text = "dòng " * 2000
    await cb.reply_code_block(msg, text, max_len=1000)
    assert len(msg.sent) > 1
    for sent_text, parse_mode in msg.sent:
        assert parse_mode == "HTML"
        assert sent_text.startswith("<pre>") and sent_text.endswith("</pre>")
